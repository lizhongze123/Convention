...
//                options.placeholder(R.drawable.image_placeholder);
                RequestOptions options = new RequestOptions().placeholder(R.drawable.image_placeholder).override(200, 200);
                Glide.with(context)
                        .asBitmap()
                        .apply(options)
                        .load(path)
...
//        }
        ...
        if (image.getPictureType().startsWith(PictureConfig.VIDEO)) {
            if (image.getDuration() >= 15000) {
                Toast.makeText(context, "视频太长", Toast.LENGTH_SHORT).show();
                return;
            }
        }
        ...
        if (selectImages.size() >= 1 && !isChecked) {
            ...
            if (image.getPictureType().startsWith(PictureConfig.VIDEO)) {
                if (videoSelectNum >= maxVideoSelectNum) {
                    String str = context.getString(R.string.picture_message_video_max_num, maxVideoSelectNum);
