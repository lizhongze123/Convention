...
//                    }
                    ...
                    if (image.getPictureType().startsWith(PictureConfig.VIDEO)){
                        if (image.getDuration() >= 15000){
                            Toast.makeText(PicturePreviewActivity.this,"视频太长",Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
                    ...
                    if (selectImages.size() >= 1 && isChecked) {
                        int videoSelectNum = videoSelectNum();
                        if (image.getPictureType().startsWith(PictureConfig.VIDEO)) {
                            if (videoSelectNum >= maxVideoSelectNum) {
                                String str = getString(R.string.picture_message_video_max_num, maxVideoSelectNum);
                                check.setSelected(false);
                                Toast.makeText(PicturePreviewActivity.this, str, Toast.LENGTH_LONG).show();
                                return;
                            }
                        } else {
                            int maxNum = videoSelectNum == 1 ? maxSelectNum - 1 : maxSelectNum;
                            if ((selectImages.size() - videoSelectNum) >= maxNum) {
                                String str = getString(R.string.picture_message_max_num, maxSelectNum);
                                check.setSelected(false);
                                Toast.makeText(PicturePreviewActivity.this, str, Toast.LENGTH_LONG).show();
                                return;
                            }
...
//                    }
    ...
    private int videoSelectNum() {
        ...
        for (int i = 0; i < selectImages.size(); i++) {
            if (selectImages.get(i).getPictureType().startsWith(PictureConfig.VIDEO)) {
                count++;
            }
        }
        return count;
    ...
    public void onSelectNumChange(boolean isRefresh) {
        ...
        if (enable) {
            tv_ok.setTextColor(Color.BLACK);
            ...
            if (numComplete) {
                ...
                tv_img_num.setText("(" + selectImages.size() + "/9)");
                tv_ok.setText("发送(" + selectImages.size() + "/9)");
            }
        } else {
