...
import com.gz0101.hzwy.material.presenter.OrderPresenterImpl;
...
public class MaterialOrderActivity extends BaseActivity<OrderPresenterImpl> implements OrderContract.MainView, View.OnClickListener {
    ...
    private ArrayList<MaterialRightBean> mList;
    ...
    protected void onCreate() {
        mList = getIntent().getParcelableArrayListExtra(MaterialConstant.KEY_GOODS_LIST);
        ...
        for (int i=0 ;i < mList.size(); i++) {
            mItemView[i] = LayoutInflater.from(this).inflate(R.layout.item_material_order, mGoodContent, false);
            initContentView(mItemView[i], mList.get(i), i);
        }
    }

    private void initContentView(View rootView, MaterialRightBean item, int index) {
    ...
    public void onClick(View v) {
        ...
        if(id == R.id.user_message_back || id == R.id.no_address) {
            onClickUser();
